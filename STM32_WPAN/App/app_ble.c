 ...
 * Advertising Data
 */
uint8_t ad_data[6] = { 5, AD_TYPE_SHORTENED_LOCAL_NAME, '0', '1', 'S', 'T', /* Shortened name */
 ...
 */
#if (P2P_SERVER1 != 0)
static const char local_name[] = { AD_TYPE_COMPLETE_LOCAL_NAME, 's', 't', 'm',
		'3', '2', 'T', 'e' };
uint8_t manuf_data[14] = { sizeof(manuf_data) - 1,
		AD_TYPE_MANUFACTURER_SPECIFIC_DATA, 0x01/*SKD version */,
...
//			Adv_Cancel_Req);
	...
	Add_Advertisment_Service_UUID(ENVIRONMENTAL_SENSING_SERVICE_UUID);//(HEALTH_THERMOMETER_SERVICE_UUID);
...
SVCCTL_UserEvtFlowStatus_t SVCCTL_App_Notification(void *pckt) {
	...
	case EVT_VENDOR:
		...
		case EVT_BLUE_GAP_PAIRING_CMPLT:
			...
			APP_DBG_MSG(
					"BLE_CTRL_App_Notification: EVT_BLUE_GAP_PAIRING_CMPLT, pairing_complete->Status = %d\n",
...
static void Ble_Hci_Gap_Gatt_Init(void) {
	...
	CONFIG_DATA_PUBADDR_LEN, (uint8_t*) bd_addr);
	...
	aci_hal_write_config_data( CONFIG_DATA_RANDOM_ADDRESS_OFFSET,
			CONFIG_DATA_RANDOM_ADDRESS_LEN, (uint8_t*) srd_bd_addr);
  ...
  role |= GAP_CENTRAL_ROLE;
	...
	if (role > 0) {
		...
		aci_gap_init(role, 0,
		CFG_GAP_DEVICE_NAME_LENGTH, &gap_service_handle,
	...
	if (aci_gatt_update_char_value(gap_service_handle,
			gap_appearance_char_handle, 0, 2, (uint8_t*) &appearance)) {
		BLE_DBG_SVCCTL_MSG("Appearance aci_gatt_update_char_value failed.\n");
	}
	/**
	 * Initialize Default PHY
	 */
	hci_le_set_default_phy(ALL_PHYS_PREFERENCE, TX_2M_PREFERRED,
			RX_2M_PREFERRED);
...
static void Adv_Request(APP_BLE_ConnStatus_t New_Status) {
	...
	if (ret == BLE_STATUS_SUCCESS) {
		if (New_Status == APP_BLE_FAST_ADV) {
			...
			HW_TS_Start(BleApplicationContext.Advertising_mgr_timer_Id,
					INITIAL_ADV_TIMEOUT);
		} else {
